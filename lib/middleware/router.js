...
module.exports = function (config) {
  ...
  var controllers = collapse(requireAll({
    dirname: path.join(config.apiDir, 'controllers'),
    filter: /(.+Controller)\.js$/,
    recursive: true
  }))

  var policies = collapse(requireAll({
  ...
  var routeMap = Object.keys(routeConfig).map(function (address, idx) {
  ...
  var policyCheck = async function (ctx, policy) {
    if (policies[policy]) {
